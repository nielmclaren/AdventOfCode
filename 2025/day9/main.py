...
def get_dir(a, b):
    ...
    print("ERROR get_dir got bad input")
...
def get_bad_coords_grid(coords, grid):
    ...
    for i, curr_coord in enumerate(coords):
        ...
        if start_dir[0] == 0:
            ...
            is_inner_corner = start_dir[1] == end_dir[0]
        else:
            # Horizontal to vertical
            is_inner_corner = start_dir[0] != end_dir[1]
...
def percentify(v):
    return f"{round(math.floor(v * 100))}%"
...
def part2(filename):
    ...
    # Keys are coord strings, e.g., "42,17" and true values indicate the cell is bad.
    sparse_grid = {}

    # Populate with the bad cells setting them to True.
    sparse_grid = get_bad_coords_grid(coords, sparse_grid)

    # Write over bad cells with the border, setting them to False and turning them back to good.
    sparse_grid = get_border_coords_grid(coords, sparse_grid)

    # Get bad cells from sparse grid keys where value is True.
    bad_coords = [str_to_coord(key) for key, value in sparse_grid.items() if value]
    ...
    print("Sorting rects by area... ", end="")
...
def main():
    ...
    #result = part2("test3.txt") # Border going right up to itself
    #result = part2("test4.txt") # Border crossing over itself
